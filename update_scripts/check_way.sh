...
if [[ "10#$current_height" -lt "10#$test_height" ]]; then
    ...
    sudo -u ${local_renderd_user} psql -d gis -c "UPDATE planet_osm_line SET wetland = 'not_flooded' WHERE osm_id = '$2';"
else
    #echo "current height is the same or more"
    sudo -u ${local_renderd_user} psql -d gis -c "UPDATE planet_osm_line SET wetland = 'flooded' WHERE osm_id = '$2';"
fi
#
