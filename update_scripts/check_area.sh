...
if [[ "$current_height" -lt "$test_height" ]]; then
    ...
    psql -d gis -c "UPDATE planet_osm_polygon SET wetland = 'not_flooded' WHERE osm_id = '$2';"
else
    #echo "current height is the same or more"
    psql -d gis -c "UPDATE planet_osm_polygon SET wetland = 'flooded' WHERE osm_id = '$2';"
fi
#
